...
import { UnionMerge, IsUnknown, IsOptional, IsIdentical } from '../TypeUtils.js';

describe('UnionMerge', () => {
	it('merges shared property type', () => {
		type R = UnionMerge<{ a: string }, { a: number }>;
		const r: IsIdentical<R, { a: string | number }> = true;
	});

	it('non shared properties becomes optional', () => {
		type R = UnionMerge<{ onlyA: number; both: boolean }, { both: boolean; onlyB: string }>;
		const r: IsIdentical<R, { onlyA?: number; both: boolean; onlyB?: string }> = true;
	});
	...
	it('merges multiple types', () => {
		...
		const r: IsIdentical<R, { a?: string; b?: number; c?: boolean }> = true;
	});
